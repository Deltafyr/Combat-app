...
# --- CONFIGURATION & CSS ---
st.set_page_config(page_title="Fight Tracker V13", page_icon="🥊", layout="wide")
...
def save_athlete(nom, titre):
    ...
    # Update or Append logic simplifiée pour gspread
    # On relit tout pour faire simple et éviter les conflits de cellules
    df = pd.DataFrame(sh.get_all_records())
    ...
    if password == "1234":
        ...
        with st.expander("👤 Gestion Titres/Bio"):
            ...
            if st.button("Sauvegarder Athlètes"):
                ...
                # On doit faire attention au format pour save_athlete mais ici on save tout le df
                client = get_client()
                try: sh = client.open("suivi_combats").worksheet("Athletes")
                except: sh = client.open("suivi_combats").add_worksheet(title="Athletes", rows=100, cols=5)
                sh.clear()
