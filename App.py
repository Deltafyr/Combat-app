...
st.set_page_config(page_title="Fight Tracker V36", page_icon="🥊", layout="wide")
...
with tab_coach:
    if st.text_input("Code", type="password") == "1234":
        ...
        with subtab_pilotage:
            ...
            live = get_live_data()
            if not live.empty:
                # ZONE DISPATCH (BOXEURS IMPORTÉS MAIS SANS NUMÉRO)
                live['Numero'] = pd.to_numeric(live['Numero'], errors='coerce').fillna(0)
                
                # Filtre : Pas terminé ET Numéro = 0
                waiting_list = live[(live['Statut'] != "Terminé") & (live['Numero'] == 0)]
                
                if not waiting_list.empty:
                    st.markdown("### ⚠️ À PROGRAMMER (Tirage)")
                    st.info("Attribuez l'Aire, le Numéro et le Casque pour les envoyer sur l'écran Live.")
                    
                    # Style pour la zone dispatch
                    ...
                    for idx, row in waiting_list.iterrows():
                        ...
                        with c_nom:
                            ...
                            st.caption(f"Catégorie: {row.get('Details_Tour', '?')}") # On utilise ce champ pour stocker la catégorie temporairement si besoin
                ...
                # ZONE PILOTAGE (BOXEURS AVEC NUMÉRO)
                        ...
                        with st.container(border=True):
                            ...
                            with c_win:
                                with st.popover("✅ GAGNÉ", use_container_width=True):
                                    st.write("Détails prochain tour :")
                            ...
                            with c_loss:
                                with st.popover("❌ DÉFAITE", use_container_width=True):
                                    ...
                                    if st.button("Terminer", key=f"e{idx}", type="primary"):
                                        ...
                                        st.rerun()
            else: st.info("Aucun boxeur dans le Live. Importez les inscrits d'abord.")
        ...
        with subtab_admin:
            ...
            if st.button("📥 Importer les Inscrits vers le Live"):
                pre = get_preinscriptions_db()
                # Filtrer sur la compétition choisie
                sub = pre[pre['Competition_Cible'] == nom_c]
                
                # Check si trouvé dans la base
                count_found = len(sub)
                ...
                if count_found > 0:
                    ...
                    for _, r in sub.iterrows():
                        # Fusion Nom + Prénom pour le Live
                        nom_complet = f"{r['Nom']} {r['Prenom']}".strip()
                        # Vérif doublon dans le Live
                        if nom_complet and (cur.empty or nom_complet not in cur['Combattant'].values):
                            rows.append({
                                ...
                                "Details_Tour": r.get('Categorie', ''), # On garde la caté en info
                                "Medaille_Actuelle":""
                            })
                    if rows: 
                        save_data(pd.concat([cur, pd.DataFrame(rows)], ignore_index=True), "Feuille 1", [])
                        st.success(f"✅ {len(rows)} boxeurs importés dans la Zone Dispatch !"); st.rerun()
                    else:
                        st.warning("Ils sont déjà tous dans le Live.")
                else: 
                    st.error(f"Aucun inscrit trouvé pour '{nom_c}' dans l'onglet 'PreInscriptions'. Vérifiez l'orthographe.")
